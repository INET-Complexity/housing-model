# -*- coding: utf-8 -*-
"""
Class to study the initial rental ask price by households when they bring their properties to the rental market. This is
the code used to create the file "InitialRentMarkUpDist.csv". It uses a summary of Zoopla rental data created with the
code at "ZooplaRentSummaryCreator.py". Note that this latter code uses a back-projected equilibrium initial rental price
for each property, based on its actual final rental price and the evolution of the RPI between the initial listing date
and the listing removal date. In this way, the markup is defined as the actual initial listing price divided by this
back-projected price.
...
filtered_data = pd.read_csv(r"./ZooplaRentSummary.csv", usecols=["MARKUP", "LOG PRICE MARKUP"])
...
with open("InitialRentPriceLog.txt", "w") as f:
...
with open("InitialRentMarkUpDist.csv", "w") as f:
...
with open("InitialRentMarkUpDistLin.csv", "w") as f:
