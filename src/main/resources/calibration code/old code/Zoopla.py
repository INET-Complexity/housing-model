...
class Zoopla:
    CSIZE = 100
    rawDaily    = pd.read_csv("W:\\Transfer\\325077\\A Raw Listings (daily).csv", chunksize=CSIZE)
    rawCollated = pd.read_csv("W:\\Transfer\\325077\\B Raw Listings (collation).csv",chunksize=CSIZE)
    matchedDaily = pd.read_csv("W:\\Transfer\\325077\\C Matched Listings (daily).csv",chunksize=CSIZE)
    matchedCollated = pd.read_csv("W:\\Transfer\\325077\\D Matched Listings (collation).csv",chunksize=CSIZE)
    matchedAggregated = pd.read_csv("W:\\Transfer\\325077\\E Matched Listings (aggregation).csv",chunksize=CSIZE)  
    matchedRegistry =  pd.read_csv("W:\\Transfer\\325077\\F Matched Listings (registry).csv",chunksize=CSIZE)
