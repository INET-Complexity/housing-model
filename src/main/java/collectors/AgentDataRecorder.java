...
//        	        									arrayDebt
    	    	...
    	    	String stringTotalWealth = cleanString(dataArray[1]);
//        	    	String stringHousingNetWealth = cleanString(dataArray[3]);
    	    	String stringConsumption = cleanString(dataArray[2]);
//        	    	String stringAnnualGrossTotalIncome = cleanString(dataArray[5]);
//        	    	String stringMonthlyDisposableIncome = cleanString(dataArray[6]);
    	    	String stringDesiredBankBalance = cleanString(dataArray[3]);
    	    	String stringBTL = cleanString(dataArray[4]);
...
//        	    	String stringDebt = cleanString(dataArray[11]);
    	    	String stringConsumptionWealth = cleanString(dataArray[5]);
...
//    		, input7, input8, input9, input10, input11, input12
        ...
        Iterator<Household> iterator = Model.households.iterator();
            ...
            double equityPositionHH = dataArray[1][h.id-oldestID] = h.getEquityPosition();
//            dataArray[3][h.id-oldestID] = equityPositionHH - bankBalanceHH;
            dataArray[2][h.id-oldestID] = h.getConsumption();
//            dataArray[5][h.id-oldestID] = h.getAnnualGrossTotalIncome();
//           dataArray[6][h.id-oldestID] = h.returnMonthlyDisposableIncome(); 
            dataArray[3][h.id-oldestID] = data.Wealth.getDesiredBankBalance(h.getAnnualGrossTotalIncome(), h.behaviour.getPropensityToSave()); //dataArray[3][h.id-oldestID] = h.behaviour.getDesiredBankBalance(h.getAnnualGrossTotalIncome());
            if(h.behaviour.isPropertyInvestor()) {
            	dataArray[4][h.id-oldestID] = 1;
            }else {dataArray[4][h.id-oldestID] = 0;}
...
//        	dataArray[11][h.id-oldestID] = h.getTotalDebt();
            dataArray[5][h.id-oldestID] = h.behaviour.getConsumptionWealth();
        	}            
        	return dataArray;
    }
