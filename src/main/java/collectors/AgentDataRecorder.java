...
//        	    	String stringDesiredBankBalance = cleanString(dataArray[7]);
        	    	String stringBTL = cleanString(dataArray[3]);
...
//        	    	String stringDebt = cleanString(dataArray[11]);
        	    	String stringConsumptionWealth = cleanString(dataArray[4]);
...
//    								double[] input11, double[] input12
    	...
    	double[][] dataArray = new double[][] {input1, input2, input3, input4, input5
...
//            dataArray[7][h.id-oldestID] = h.behaviour.getDesiredBankBalance(h.getAnnualGrossTotalIncome());
            if(h.behaviour.isPropertyInvestor()) {
            	dataArray[3][h.id-oldestID] = 1;
            }else {dataArray[3][h.id-oldestID] = 0;}
...
//        	dataArray[11][h.id-oldestID] = h.getTotalDebt();
            dataArray[4][h.id-oldestID] = h.behaviour.getConsumptionWealth();
        	}            
        	return dataArray;
    }
